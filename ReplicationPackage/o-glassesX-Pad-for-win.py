...
#
#基本的な使い方
#オ#プション無し：datasetディレクトリ(-d)のデータを使ってk-分割交差検証の実験
#-omオプション：学習済みモデルを出力
...
def main():
        ...
        if not args.input_model:
                ...
                for f in files_file:
                        ...
                        joint=f.split("/")[1] # JOINT
                        compiler=f.split("/")[1].split("_")[0]
                        opt=f.split("/")[1].split("_")[1]
                ...
                for f in files_file:
                        ...
                        if args.disasm_x86:
                                ...
                                for (offset, size, instr, hexdump) in l:
                                        ...
                                        token = instrInPadding(instr,hexdump)
                                        if token == 'INS':
                                                tok = b'\1' #b'001'
                                        elif token == 'DB':
                                                tok = b'\2' #b'010'
                                        ...
                                        if token != 'INS': 
                                                ...
                                                if size>16:
                                                        ...
                                                        b += tok+bdata[pos:pos+16-1]
                                                        #bi.append(bdata[pos:pos+16])
                                                else:
                                                        #b += (offset).to_bytes(4, byteorder="little")+bdata[pos:pos+size]+b'\0'*(16-size)
                                                        #b += bdata[pos:pos+size]+b'\0'*(16-size)
                                                        b += tok+bdata[pos:pos+size]+b'\0'*(16-size-1)
                                                        #print("bdata pos: ",bdata[pos:pos+size])
                                                        #exit()
                                                        bi.append(instr+'0'*(16-size))
                                                        #print(bi)
                                                        #exit()
                        ...
                        #print("block size: ",block_size) # 1024, we need a black of 128 since we have bytes
                        #print(bidata[0:2])
                        #exit()
                        offset_count = 0
...
#                                        train.extend(BitArray[ord(x)])
                                ...
                                master_dataset_f.append((block,ftype,f))
                                num_of_dataset[ft]+=1
...
#                        b = ord(bdata[i])
                ...
                #解析対象のファイルの分類結果を表示
                ###print("img len",len(img_))
                print("range col",range(len(col)))
                img = [Image.new('RGB', (128, h)) for i in range(len(col))]
                print("len img",len(img))
                l=1
                print("num_types",num_of_types)
                ...
                for c in range(0,fsize-block_size+1,l):
                        ...
                        if args.disasm_x86:
                                ...
                                block = b''
                                for i in m:      # only collect the padding instructions as the model is only built on padding instruction
                                        #exit()
                                        #print("************************** Prediction ***********************************************")
                                        #print("i[3]: %-16s",i[3])
                                #for (offset, size, instr, hexdump) in m:
                                        if instrInPadding(i[2],i[3]):
                                                print(i[0])
                                                print(i[1])
                                                print(i[2])
                                                print(i[3])
                                                ...
                                                for c_ in range(16):#16バイトで命令を切る
                                                        if c_ < len(i[3])/2:
                                                                #b += (chr(int(i[3][c_*2:c_*2+2],16))).encode() #TypeError: can't concat str to bytes (ascii cannot decode all bytes'
                                                                b += bytes(chr(int(i[3][c_*2:c_*2+2],16)),'utf-8')
                                                        else:
                                                                b += b'\0'
                                                block += b
                ...
                if output_image:
                    ...
                    #img.show()
                '''
        elif args.input: # given a file for inference using a model
                ...
                for i in range(0,fsize):
                        ...
                        img.putpixel((i%128,math.floor(i/128)),c)
                #exit()
                if output_image:
                    for num in range(0,math.floor((h-1)/max_h+1)):
                            box = (0,num*max_h,128,num*max_h+max_h)
                            img.crop(box).save(basename+"_bitmap_"+"{0:04d}".format(num)+".png")
                    box = (0,num*max_h,128,h)
                    img.crop(box).save(basename+"_bitmap_"+"{0:04d}".format(num)+".png")
                ...
                num=0
                asm = {}
                print("loop ending at: ",fsize-block_size+1)
                print("loop jump size: ",l)
                #exit()
                for c in range(0,fsize-block_size+1,l):
                        offset = c*1.0/fsize
                        print("offset: ", offset)
                        ...
                        e = entropy(block_)
                        print("entropy of block: ",e)
                        for j in range(0,l):
                                img_.putpixel(((c+j)%128,math.floor((c+j)/128)),(e,e,e))
                        if args.disasm_x86:
                                        ...
                                        if instrInPadding(i[2],i[3]):
                                                ...
                                                for c_ in range(16):
                                                        if c_ < len(i[3])/2:
                                                                ...
                                                                #print(i[3][c_*2:c_*2+2]) 
                                                                b += str.encode(i[3][c_*2:c_*2+2])
                                                                #exit()
                                                                #b += ord(chr(int(i[3][c_*2:c_*2+2],16)))
                                                        else:
                                                                b += b'\0'
                                                block += b
                                block = block[:block_size]
                        ...
                        with chainer.using_config('train', False):
                                ...
                                if False and result3 > 0.99 and file_types_[result2] in args.input:
                                        ...
                                        if m[ai][2] in asm:
                                                ...
                                                asm[m[ai][2]]=1
                        #print("This did not come:*****************************")
                        for j in range(0,l):
                                for i in range(len(col)):
                                        img[i].putpixel(((i_*l+j)%128,math.floor((i_*l+j)/128)),col[i][result2])
                        i_+=1
                        print("i_ ", i_)
                        #exit()
